...
-- Prevent duplicate injection (executor-ready)
if PlayerGui:FindFirstChild("KeySystemGUI") then return end
...
local Config = {
    ...
    ParticleCount = 60,
    ParticleSpeed = 60,
    -- Single local key (user requested)
    LocalKey = "A9X4B7C2D6E1",
    -- Launcher URL
    LauncherURL = "loadstring(game:HttpGet("https://raw.githubusercontent.com/Tydetoysf/Projectlnfra/main/arsenal.lua",
    -- Get key link
    GetKeyLink = "https://discord.gg/cG6VUgKnzU"
}
...
-- (kept the structure from the original)
-- ========================================
...
local function RunRemoteScriptSafe(url)
...
-- Advanced fade-out: fades UI, scales and rotates container, fades particles, then destroys
local function AdvancedFadeOutAndClose()
    if State.IsDestroyed or not UI.ScreenGui then return end
    State.IsDestroyed = true

    -- stop spinners & animations
    pcall(function()
    ...
    -- Fade backdrop fully
    local t1 = Services.TweenService:Create(UI.Backdrop, TweenInfo.new(0.5, Enum.EasingStyle.Quad),
        {BackgroundTransparency = 1})
    t1:Play()

    -- scale & rotate container out while increasing transparency for child elements
    local scaleTween = Services.TweenService:Create(UI.Container, TweenInfo.new(0.6, Enum.EasingStyle.Back, Enum.EasingDirection.In),
        {Size = UDim2.new(0, 10, 0, 10), BackgroundTransparency = 1})
    scaleTween:Play()

    -- rotate using a manual loop (UI elements don't have Rotation property, so rotate child frames we can)
    local rotateTweenTime = 0.6
    local start = tick()
    local children = UI.Container:GetDescendants()
    -- Fade out text labels and frames
    for _,child in ipairs(children) do
        if child:IsA("TextLabel") or child:IsA("TextButton") or child:IsA("Frame") or child:IsA("ImageLabel") then
            pcall(function()
                Services.TweenService:Create(child, TweenInfo.new(0.45, Enum.EasingStyle.Quad),
                    {BackgroundTransparency = 1, TextTransparency = child:IsA("TextLabel") and 1 or nil}):Play()
            end)
        end
    end
    ...
    task.spawn(function()
        for i=#State.Particles,1,-1 do
            ...
            if p and p.frame then
                pcall(function()
                    Services.TweenService:Create(p.frame, TweenInfo.new(0.5, Enum.EasingStyle.Quad), {BackgroundTransparency = 1}):Play()
                end)
                p.frame:Destroy()
                table.remove(State.Particles, i)
            end
...
-- SubmitKey tries server RemoteFunction first; falls back to local key
local function SubmitKey(key)
    if State.IsLoading then return false end
    SetLoading(true)
    ShowStatus("Validating key...", false, false)

    local remote = Services.ReplicatedStorage:FindFirstChild("KeyValidation")
    if remote and remote:IsA("RemoteFunction") then
        ...
        SetLoading(false)
        if not ok then
            ShowStatus("Validation failed (server). Try again.", true)
            return false
        end

        -- server result can be boolean or table { valid = true, reason = "..."}
        if type(result) == "table" then
            if result.valid then
                ShowStatus("Key accepted — launching...", false, true)
                local ok2 = RunRemoteScriptSafe(Config.LauncherURL)
                if not ok2 then ShowStatus("Failed to run remote script.", true); return false end
                AdvancedFadeOutAndClose()
                return true
            else
                ShowStatus("Invalid key: " .. (result.reason or "Denied"), true)
                return false
            end
        elseif type(result) == "boolean" then
            if result then
                ...
                ShowStatus("Invalid key (server).", true)
                return false
            end
        else
    ...
    -- Local fallback (single allowed key)
    SetLoading(false)
    if key == Config.LocalKey then
        ShowStatus("Key accepted (local) — launching...", false, true)
        local ok2 = RunRemoteScriptSafe(Config.LauncherURL)
        if not ok2 then ShowStatus("Failed to run remote script.", true); return false end
        AdvancedFadeOutAndClose()
        return true
    else
        ShowStatus("Invalid key (local).", true)
        return false
    end
end

-- ========================================
-- EVENT CONNECTIONS (modified to call SubmitKey)
-- ========================================
...
local function ConnectEvents()
    ...
    UI.Buttons.GetKey.MouseButton1Click:Connect(function()
        ...
        CopyToClipboard(Config.GetKeyLink, "Key website link copied to clipboard!")
    end)
